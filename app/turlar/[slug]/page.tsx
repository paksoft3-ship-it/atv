...
export default function TourDetailPage({ params }: Props) {
  ...
  return (
    <div className="bg-[#112215] min-h-screen text-white">
      {/* Hero */}
      <div className="relative h-[85vh] w-full overflow-hidden">
        ...
        >
          <div className="absolute inset-0 bg-gradient-to-t from-[#112215] via-[#112215]/50 to-transparent" />
          <div className="absolute inset-0 bg-[#112215]/30" />
        </div>
        <div className="relative z-10 flex h-full flex-col justify-end pb-20 px-4 md:px-20 max-w-6xl mx-auto">
          <div className="max-w-4xl">
            <div className="flex items-center gap-2 mb-4">
              ...
              <span className="text-slate-300 text-sm">({tourReviews.length + 120}+ Değerlendirme)</span>
            </div>
            <h1 className="text-white text-5xl md:text-7xl font-black leading-[0.95] tracking-tight mb-6">
              {tour.name}
            </h1>
            <p className="text-slate-200 text-lg md:text-xl leading-relaxed max-w-2xl mb-8">
      ...
      {/* Stats Bar */}
      <div className="bg-[#1a3320] border-y border-[#23482c] py-8">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            {[
              ...
              <div key={stat.label} className="flex items-center gap-4">
                <div className="size-12 rounded-full bg-accent/10 flex items-center justify-center text-accent shrink-0">
      ...
      <div className="max-w-6xl mx-auto px-4 md:px-20 py-16 flex flex-col lg:flex-row gap-12">
        ...
        <div className="flex-1 flex flex-col gap-16">
          ...
          <section>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-8">
              {tour.highlights.map((highlight) => (
                <div
                  key={highlight.title}
                  className="bg-[#1a3320] p-4 rounded-xl border border-[#23482c] flex items-start gap-3"
                >
                  <span className="material-symbols-outlined text-accent mt-1">check_circle</span>
                  <div>
                    <h4 className="text-white font-bold text-sm">{highlight.title}</h4>
          ...
          {/* Route Section */}
          <section className="flex flex-col md:flex-row gap-8 items-center bg-[#1a3320] rounded-2xl p-6 md:p-8 border border-[#23482c]">
            <div className="flex-1 space-y-4">
              <h3 className="text-2xl font-bold text-white">{tour.route.title}</h3>
              <p className="text-slate-300">{tour.route.description}</p>
              <ul className="space-y-2">
                {tour.route.waypoints.map((wp) => (
                  <li key={wp} className="flex items-center gap-2 text-slate-300 text-sm">
          ...
          <section className="flex flex-col md:flex-row-reverse gap-8 items-center">
            <div className="flex-1 space-y-4">
              <h3 className="text-2xl font-bold text-white">{tour.safety.title}</h3>
              <p className="text-slate-300">{tour.safety.description}</p>
              <Link
                href="/sss"
                className="text-accent font-bold text-sm flex items-center gap-1 hover:text-white transition-colors"
              >
            ...
            {tour.safetyImage && (
              <div className="w-full md:w-1/2 h-64 rounded-xl overflow-hidden relative border border-[#23482c]">
                <Image
      ...
      {/* Reviews */}
      <div className="border-t border-[#23482c] bg-[#1a3320] py-20">
        <div className="max-w-6xl mx-auto px-4 md:px-20">
          <div className="flex flex-col md:flex-row justify-between items-end mb-12 gap-6">
            <div>
              ...
              <p className="text-slate-400">Son 12 ayda 500+ mutlu maceracı.</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {testimonialsData.slice(0, 3).map((review) => (
              <div
                key={review.id}
                className="bg-[#112215] p-6 rounded-2xl border border-[#23482c]"
              >
                ...
                <div className="flex items-center gap-3">
                  <div className="size-10 rounded-full bg-slate-700 flex items-center justify-center text-sm font-bold">
                    {review.initials}
                  </div>
                  <div>
      ...
      {/* Mobile Booking Bar */}
      <div className="lg:hidden fixed bottom-0 left-0 right-0 z-50 bg-[#112215] border-t border-[#23482c] p-4 flex items-center justify-between shadow-xl">
        <div>
          <p className="text-slate-400 text-xs font-medium">Başlangıç Fiyatı</p>
